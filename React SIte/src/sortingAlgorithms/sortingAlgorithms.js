...
export function getMergeSortAnimations(array) {
  ...
  function doMerge(
    ...
    let j = middleIdx + 1;
    while (i <= middleIdx && j <= endIdx) {
      animations.push([i, j]); // grab compared items and chage color(moving) 
      animations.push([i, j]); // grab compared items and revert color
      if (auxArray[i] <= auxArray[j]) {
        animations.push([k, auxArray[i]]); // overwrite main[k] with aux[i]
        mainArray[k++] = auxArray[i++];
      } else {
        animations.push([k, auxArray[j]]); // overwrite main[k] with aux[j]
        mainArray[k++] = auxArray[j++];
      }
    }
    while (i <= middleIdx) {
      animations.push([i, i]);//grab compared items and change color(moving) 
      animations.push([i, i]);//compare and revert color
      animations.push([k, auxArray[i]]); // overwrite main[k] with aux[j]
      mainArray[k++] = auxArray[i++];
    }
    while (j <= endIdx) {
      animations.push([j, j]); //grab compared items and change color (moving) 
      animations.push([j, j]); //compare and revert color
      animations.push([k, auxArray[j]]); // overwrite main[k] with aux[j]
      mainArray[k++] = auxArray[j++]; 
    }
  }